  ...
  <body class="bg-awal">
    ...
    <div class="pt-5 mt-5 bg-awal w-100 d-flex justify-content-center align-items-center" style="height: 30vh">
      <div class="content text-md-center text-start mx-4 mx-md-0" data-aos="zoom-in" data-aos-duration="1300">
        <h1 class="fs-60 text-info font-payback">Transaksi</h1>
      </div>
    </div>
    <!--  judul end -->
      ...
      <div class="row">
        <div class="col-md-2 bg-awal h-100">
          <ul class="nav flex-column mt-2 ms-2">
            ...
            <li class="nav-item">
              <a class="nav-link text-info border-bottom border-secondary my-3" href="transaksi.php">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-cash-coin text-info me-2" viewBox="0 0 16 16">
          ...
          <div class="card p-2">
            <div class="table-responsive">
              <table class="table table-bordered">
                ...
                <tbody>
                  <?php $i = 1;?>
                  <?php foreach($transaksi as $transaksi2):?>
                  <tr>
                    <td class="text-center"><?= $i;?></td>
                    <td><?= $transaksi2["tanggal"];?></td>
                    <td>
                      <?php 
                      $user = $transaksi2['id_user'];
                      $user2 = query("SELECT * FROM user WHERE id_user = '$user'")[0];
                      ?>
                      <?= $user2['nama'];?>
                    </td>
                    <td>
                      <?php 
                      $kelas = $transaksi2['id_kelas'];
                      $kelas2 = query("SELECT * FROM kelas WHERE id_kelas = '$kelas'")[0];
                      ?>
                      <?= $kelas2['nama_kelas'];?>
                    </td>
                    <td>Rp. <?= $transaksi2["total_bayar"];?></td>
                    <td><img src="img/<?= $transaksi2["gambar"];?>" alt="" width="150"></td>
                    <td>
                      <a href="hapustransaksi.php?id_transaksi=<?= $transaksi2["id_transaksi"];?>" class="text-decoration-none mb-3 btn btn-danger" onclick="return confirm('Anda yakin data ingin dihapus ?');" data-bs-toggle="tooltip" title="Hapus">
